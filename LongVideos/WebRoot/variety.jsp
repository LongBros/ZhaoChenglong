...
if("other".equals(cate)){
	...
	cateI=cate;
	cateName=Movie.getCatgoryNameById("variety", cateI);
...
<html>
  <head>
    <title>综艺-<%=cateName%>(分类)-553影院</title>
  ...
  <body>
		<ul class="layui-nav layui-bg-green">
		  ...
		  <li class="layui-nav-item layui-this"><a href="/variety.jsp">综艺</a></li>
	...
	<%
		...
		out.write("类型:&nbsp;");
		ArrayList<Category> cs=Movie.getCate("zongyi");//根据分类获取分类下的类型
		for(Category c:cs){
			String cat=c.getId();
			...
			if(name.contains(cateName)){//因name中可能含有空格，故不用equal
				url="<a href='/variety.jsp?cate="+cat+"&page=1'><font color='orange'>"+name+"</font></a>&nbsp;";
			}else{
				url="<a href='/variety.jsp?cate="+cat+"&page=1'>"+name+"</a>&nbsp;";
			}
			out.write(url);
		}
		out.write("</div>");	
		String main="https://www.360kan.com";
        String url="https://www.360kan.com/zongyi/list.php?rank=rankhot&cat="+cateI+"&area=all&year=all&pageno="+pageI;
		...
		for(Element play: plays){
			...
			out.write("<div class='whole'><a href=\"/player.jsp?type=zongyi&url="+hre+"\" target='_blank'>"
			  		+ "<img src='"+src+"' title='"+name+"' alt='"+src+"'>"
			  				+ "<span>"+name+"</span><br><p>"+desc+"</p></a></div>");
		   }
			out.write("<center><div class='page'>");
			if(pageI>1){//当前页码大于1，显示上一页按钮
				out.write("<a  href=\"/variety.jsp?cate="+cate+"&page="+(pageI-1)+"\">上</a>");
			}
			if(pageI>24-6){//当前页码大于总页码-6，输出后六页
			    for(int j=24-5;j<=24;j++){
			        if(j==pageI){
			             ...
			             out.write("<a href=\"/variety.jsp?cate="+cate+"&page="+j+"\">"+j+"</a>"); 
			        }
			    }
			...
			if(pageI<24){//当前页码小于24，显示下一页按钮
				out.write("<a  href=\"/variety.jsp?cate="+cate+"&page="+(pageI+1)+"\">下</a>");
			}
