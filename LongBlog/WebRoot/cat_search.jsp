...
String l=request.getParameter("id");//分类id
CategoryDao c=new CategoryDao();
String na=c.queryCatNameById(Integer.parseInt(l));//分类名
...
<html>
  <head>
    <base href="<%=basePath%>">
    <title><%=na%>(分类)-Long Bro博客</title>
	<meta http-equiv="pragma" content="no-cache">
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="expires" content="0">    
	<meta http-equiv="keywords" content="<%=na%>,Long Bro博客,Long Bro">
	<meta http-equiv="description" content="Long Bro博客中关于“<%=na%>”分类的所有博客，欢迎大家阅读评论...Long Bro博客欢迎你.">
        ...
        <%
	       ...
	       //将所有的页码添加至数组链表
			...
			for(int i=0;i<arr1.size();i++){
             ...
             int num=dao.queryBlogsNum(" where cat_Id="+l);
             int pages=0;//博客页数
             //每页五篇，计算出页数
             if(num%5==0){
                pages=num/5;
             ...
             String sel="select * from blogs where cat_Id="+Integer.parseInt(l)+" order by b_Id desc limit "+((pageI-1)*5)+",5";
  ...
  <%
    if(pageI>1){
       %>
         <a href="/cat_search.jsp?id=<%=l %>&pageId=<%=pageI-1%>">上</a>
       <%
    }
   %>
  <%
     if(pages<=6){//页数小于等于6，直接输出6个页数
        for(int j=1;j<=pages;j++){
         if(j==pageI){
              ...
              out.write("<a href=\"/cat_search.jsp?id="+l+"&pageId="+j+"\">"+j+"</a>"); 
         }
       } 
  ...
  %>
   ...
   if(pageI<pages){
      %>
          <a href="/cat_search.jsp?id=<%=l %>&pageId=<%=pageI+1%>">下</a>
      <%
   }
    %>
  </div>
